...
export function getAssuranceSnapshot(): AssuranceSnapshot {
  ...
  return { source: { path: "doc/checkout-requirement.md", hash: hash(source), title: source.match(/^# (.+)$/m)?.[1] ?? "Checkout Release Requirement" }, tests, evidencePacks, coverage: { designed, proven, failed, owed, percentage: designed ? Math.round((proven / designed) * 100) : 0 }, ledgerBalance: balances.at(-1)?.[1] ?? "785.9054", runs };
}
